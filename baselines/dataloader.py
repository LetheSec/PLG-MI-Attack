...
# 					img_list.append(img)
# 			except:
# 				# print("somethe wrong at ", img_name)
# 				continue
# 		return img_list
...
for path in os.listdir(root):
	for _, _, files in os.walk(os.path.join(root, path)):
		for img_name in files:
			images_path.append(os.path.join(root, path, img_name))
images_path=sorted(images_path)

image_list = ['/'.join(image.split('/')[-1:]) for image in images_path]

file_write_obj = open("covid19.txt", 'w')
...
# 	"""Some Information about FFHQ"""
# 	def __init__(self, root='/data-x/g12/yuanxiaojian/datasets/FFHQ/thumbnails128x128/'):
# 		super(FFHQ, self).__init__()
# 		self.root = root

# 		self.images_path = []
#         for path in os.listdir(self.root):
#             for _, _, files in os.walk(os.path.join(self.root, path)):
#                 for img_name in files:
#                     self.images_path.append(os.path.join(self.root, path, img_name))


# 		self.images_path = sorted(self.images)
# 		self.num_img = len(self.images_path)
# 		self.image_list = self.load_img()
# 		print("Load " + str(self.num_img) + " images")
...
# 	def load_img(self):
# 		img_list = []
# 		for i, img_name in enumerate(self.images_path):
# 			if img_name.endswith(".png") or img_name.endswith(".jpg"):
# 				img = PIL.Image.open(path)
# 				img = img.convert('RGB')
# 				img_list.append(img)
